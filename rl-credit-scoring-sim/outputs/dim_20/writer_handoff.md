...
- This handoff corresponds to the `20D` pipeline output written under `outputs/dim_20`.
...
- State definition: 20-dimensional observation. The first 12 ordered features remain the baseline features: `week_progress`, `approval_rate_current`, `approval_rate_new`, `approval_rate_repeat`, `rolling_realized_default_rate`, `expected_default_rate_current`, `realized_profit_scaled`, `rolling_profit_volatility_scaled`, `projected_capital_usage_ratio`, `outstanding_ratio`, `threshold_new_normalized`, `threshold_repeat_normalized`.
- Added beyond the baseline for this run: `repeat_share_current`, `expected_profit_per_application_scaled`, `expected_npv_per_application_scaled`, `realized_npv_scaled`, `weekly_reward_scaled`, `threshold_gap_normalized`, `capital_headroom_ratio`, `realized_expected_default_gap`. The cumulative layer definitions are recorded in `state_dimension_manifest.md`.
...
- Best RL controller: `double_dqn` with cumulative reward mean 69812.02 and expected profit mean 61096.44.
- Scenario winners:
- `adverse_stress`: `profit_oriented` with expected profit mean 29336.42
- `base_market`: `double_dqn` with expected profit mean 94321.51
...
- Diagnostic results:
- `dqn` in `adverse_stress`: early gap -3832.40, final gap 53409.49
- `double_dqn` in `adverse_stress`: early gap -526.03, final gap 209167.86
- `ppo` in `adverse_stress`: early gap -2010.07, final gap 114364.76
- `sac` in `adverse_stress`: early gap -2128.13, final gap 97969.42
