...
- This handoff corresponds to the `50D` pipeline output written under `outputs/dim_50`.
...
- Best RL controller: `dqn` with cumulative reward mean 78198.39 and expected profit mean 66946.81.
- Scenario winners:
- `adverse_stress`: `profit_oriented` with expected profit mean 29336.42
- `base_market`: `dqn` with expected profit mean 92105.55
...
- Diagnostic results:
- `dqn` in `adverse_stress`: early gap -1185.37, final gap 235141.85
- `double_dqn` in `adverse_stress`: early gap -3162.98, final gap 26807.00
